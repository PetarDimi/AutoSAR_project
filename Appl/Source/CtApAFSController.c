...
FUNC(void, CtApAFSController_CODE) RCtApAFSControllerLogic(void) /* PRQA S 0850 */ /* MD_MSR_19.8 */
{
	...
	Rte_Read_PpAmbientLight_DeAmbientLight(&ambientLightValue);
	Rte_Read_PpSpeedometerValue_DeSpeedometerValue(&speedometerValue);
	Rte_Read_PpBateryMode_DeBateryMode(&batteryMode);
	...
	if (batteryMode != 0)
	{
		...
		{
			if (ambientLightValue <= AMBIENT_LIGHT_TRESHOLD)
			{
				...
				else if (speedometerValue > 50 && speedometerValue <= 120)
				{
					/* COUNTRYMODE */
					/* TODO */
	...
	if (TRUE == highBeamModeSwitchState)
	{
